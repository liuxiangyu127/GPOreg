...
#function for fitting the transformed linear model with the GPO
  ...
  fit <- lm(as.formula(paste0("etaihat ~", as.character(formula)[[3]])), data = data)
  coefs <- coef(fit)
  var.naive <- vcov(fit)
  ...
  Y.ij2 <- apply(Ys, 2, function(x) (matrix(rep(x, n), nrow = n,
                                            byrow = T) < matrix(rep(x, n), nrow = n, byrow = F)) + 1/n)
  Y.ij <- (Y.ij1 + Y.ij2)/2
  ksi.ij <- matrix(rowMeans(Y.ij), nrow = n) - pihat
  s2i.ij <- array(NA, c(n, n, K.x + 1))
  for (p in 1:(K.x + 1)) s2i.ij[, , p] <- Zmat[, p] * g.deriv * ksi.ij
  ...
  # names(result) <- c("Estimate", "StdErr", "Z", "p")
  result <- list(coef = coefs, var = var.ABA, z.value = z.value, p.value = p.value, call = call)
  class(result) <- "GPOreg"
  return(result)
}
